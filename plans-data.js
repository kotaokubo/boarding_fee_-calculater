// Plans data separated for easier maintenance and potential future replacement with external JSON.
// New structure: Each plan contains basePrice (per-person pricing), charter (minimum pricing), rental, and note.
const plans = {
  "午前アジ": {
    "basePrice": { "men": 6800, "women": 6000, "student": 3800 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 54400 },
      "holiday": { "minPeople": 15, "minPrice": 102000 },
      "sunday": { "minPeople": 12, "minPrice": 81600 }
    },
    "rental": { "竿（通常竿, リール）": 600, "ビシセット": { "price": 2200, "refund": 2000 } },
    "note": "コマセ・イカ短・アオイソ・氷付き",
    "visibleShared": true,
    "visibleCharter": true
  },
  "午後アジ": {
    "basePrice": { "men": 6800, "women": 6000, "student": 3800 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 54400 },
      "holiday": { "minPeople": 12, "minPrice": 81600 },
      "sunday": { "minPeople":  10, "minPrice": 68000 }
    },
    "rental": { "竿（通常竿, リール）": 600, "ビシセット": { "price": 2200, "refund": 2000 } },
    "note": "コマセ・イカ短・アオイソ・氷付き",
    "visibleShared": true,
    "visibleCharter": true
  },
  "午前キス": {
    "basePrice": { "men": 6800, "women": 6000, "student": 3800 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 54400 },
      "holiday": { "minPeople": 15, "minPrice": 102000 },
      "sunday": { "minPeople": 12, "minPrice": 81600 }
    },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "午前カサゴ": {
    "basePrice": { "men": 6800, "women": 6000, "student": 3800 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 54400 },
      "holiday": { "minPeople": 15, "minPrice": 102000 },
      "sunday": { "minPeople": 12, "minPrice": 81600 }
    },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "午後キス": {
    "basePrice": { "men": 6800, "women": 6000, "student": 3800 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 54400 },
      "holiday": { "minPeople": 12, "minPrice": 81600 },
      "sunday": { "minPeople": 10, "minPrice": 68000 }
    },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "午後カサゴ": {
    "basePrice": { "men": 6800, "women": 6000, "student": 3800 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 54400 },
      "holiday": { "minPeople": 12, "minPrice": 81600 },
      "sunday": { "minPeople": 10, "minPrice": 68000 }
    },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "午前マダコ": {
    "basePrice": { "men": 6800, "women": 6000, "student": 3800 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 54400 },
      "holiday": { "minPeople": 15, "minPrice": 102000 },
      "sunday": { "minPeople": 12, "minPrice": 81600 }
    },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "午前アミ五目": {
    "basePrice": { "men": 7200, "women": 6500, "student": 4300 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 57600 },
      "holiday": { "minPeople": 15, "minPrice": 108000 },
      "sunday": { "minPeople": 12, "minPrice": 86400 }
    },
    "rental": { "竿（通常竿, リール）": 600, "ビシセット": { "price": 1800, "refund": 1700 } },
    "note": "アミコマセ・イカ短・付け餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "午後アミ五目": {
    "basePrice": { "men": 7200, "women": 6500, "student": 4300 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 57600 },
      "holiday": { "minPeople": 12, "minPrice": 86400 },
      "sunday": { "minPeople": 10, "minPrice": 72000 }
    },
    "rental": { "竿（通常竿, リール）": 600, "ビシセット": { "price": 1800, "refund": 1700 } },
    "note": "アミコマセ・イカ短・付け餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "午前マゴチ": {
    "basePrice": { "men": 7200, "women": 6500, "student": 4300 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 57600 },
      "holiday": { "minPeople": 15, "minPrice": 108000 },
      "sunday": { "minPeople": 12, "minPrice": 86400 }
    },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "サイマキ５匹・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "午後マゴチ": {
    "basePrice": { "men": 7200, "women": 6500, "student": 4300 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 57600 },
      "holiday": { "minPeople": 12, "minPrice": 86400 },
      "sunday": { "minPeople":  10, "minPrice": 72000 }
    },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "サイマキ５匹・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "午前タチウオ": {
    "basePrice": { "men": 7200, "women": 6500, "student": 4300 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 57600 },
      "holiday": { "minPeople": 15, "minPrice": 108000 },
      "sunday": { "minPeople": 12, "minPrice": 86400 }
    },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "午前メバル": {
    "basePrice": { "men": 7200, "women": 6500, "student": 4300 },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "活きモエビ・アオイソメ・氷付き",
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 57600 },
      "holiday": { "minPeople": 15, "minPrice": 108000 },
      "sunday": { "minPeople": 12, "minPrice": 86400 }
    },
    "visibleShared": false,
    "visibleCharter": false
  },
  "午後エギイカ": {
    "basePrice": { "men": 6800, "women": 6000, "student": 3800 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 54400 },
      "holiday": { "minPeople": 12, "minPrice": 81600 },
      "sunday": { "minPeople": 10, "minPrice": 68000 }
    },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "マダイ五目": {
    "basePrice": { "men": 12200, "women": 10500, "student": 7500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 97600 },
      "holiday": { "minPeople": 10, "minPrice": 122000 },
      "sunday": { "minPeople": 10, "minPrice": 122000 }
    },
    "rental": {
      "竿（通常竿, リール）": 1200,
      "竿（電動リール）": 2500,
      "ビシセット": { "price": 3700, "refund": 3500 }
    },
    "note": "オキアミ規定量・氷付き",
    "visibleShared": true,
    "visibleCharter": true
  },
  "ＬＴイサキ五目": {
    "basePrice": { "men": 12200, "women": 10500, "student": 8000 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 97600 },
      "holiday": { "minPeople": 12, "minPrice": 146400 },
      "sunday": { "minPeople": 12, "minPrice": 146400 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "アミコマセ・イカ短・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "ヤリ・スルメイカ": {
    "basePrice": { "men": 12500, "women": 11000, "student": 8000 },
    "charter": {
      "weekday": { "minPeople": 10, "minPrice": 125000 },
      "holiday": { "minPeople": 12, "minPrice": 150000 },
      "sunday": { "minPeople": 12, "minPrice": 150000 }
    },
    "rental": {
      "竿（通常竿, リール）": 1200,
      "竿（電動リール）": 2500
    },
    "note": "氷付き、投入機あり",
    "visibleShared": true,
    "visibleCharter": true
  },
  "ヤリイカ": {
    "basePrice": { "men": 12500, "women": 11000, "student": 8000 },
    "charter": {
      "weekday": { "minPeople": 10, "minPrice": 125000 },
      "holiday": { "minPeople": 12, "minPrice": 150000 },
      "sunday": { "minPeople": 12, "minPrice": 150000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "氷付き、投入機あり",
    "visibleShared": false,
    "visibleCharter": false
  },
  "スルメイカ": {
    "basePrice": { "men": 12500, "women": 11000, "student": 8000 },
    "charter": {
      "weekday": { "minPeople": 10, "minPrice": 125000 },
      "holiday": { "minPeople": 12, "minPrice": 150000 },
      "sunday": { "minPeople": 12, "minPrice": 150000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "氷付き、投入機あり",
    "visibleShared": false,
    "visibleCharter": false
  },
  "ワラサ": {
    "basePrice": { "men": 12000, "women": 10000, "student": 9500 },
    "charter": {
      "weekday": { "minPeople": 10, "minPrice": 120000 },
      "holiday": { "minPeople": 12, "minPrice": 144000 },
      "sunday": { "minPeople": 12, "minPrice": 144000 }
    },
    "rental": {
      "竿（通常竿, リール）": 1200,
      "竿（電動リール）": 2500
    },
    "note": "オキアミ規定量・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "タチアジリレー": {
    "basePrice": { "men": 11500, "women": 9500, "student": 7000 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 92000 },
      "holiday": { "minPeople": 10, "minPrice": 115000 },
      "sunday": { "minPeople": 10, "minPrice": 115000 }
    },
    "rental": {
      "竿（通常竿, リール）": 1200,
      "竿（電動リール）": 2500
    },
    "note": "餌・ミンチ・氷付き",
    "visibleShared": true,
    "visibleCharter": true
  },
  "タチウオのみ": {
    "basePrice": { "men": 10500, "women": 8500, "student": 6500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 84000 },
      "holiday": { "minPeople": 10, "minPrice": 105000 },
      "sunday": { "minPeople": 10, "minPrice": 105000 }
    },
    "rental": {
      "竿（通常竿, リール）": 1200,
      "竿（電動リール）": 2500
    },
    "note": "餌・ミンチ・氷付き",
    "visibleShared": false,
    "visibleCharter": true
  },
  "遠征ウィリー五目": {
    "basePrice": { "men": 12500, "women": 11000, "student": 8000 },
    "charter": {
      "weekday": { "minPeople": 10, "minPrice": 125000 },
      "holiday": { "minPeople": 12, "minPrice": 150000 },
      "sunday": { "minPeople": 12, "minPrice": 150000 }
    },
    "rental": {
      "竿（通常竿, リール）": 1200,
      "竿（電動リール）": 2500
    },
    "note": "アミコマセ・付け餌・氷付き",
    "visibleShared": true,
    "visibleCharter": true
  },
  "カワハギ": {
    "basePrice": { "men": 11500, "women": 10000, "student": 7000 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 92000 },
      "holiday": { "minPeople": 10, "minPrice": 115000 },
      "sunday": { "minPeople": 10, "minPrice": 115000 }
    },
    "rental": {
      "竿（専用竿, リール）": 1500
    },
    "note": "アサリ餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "マゴチ": {
    "basePrice": { "men": 11000, "women": 9000, "student": 7000 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 88000 },
      "holiday": { "minPeople": 10, "minPrice": 110000 },
      "sunday": { "minPeople": 10, "minPrice": 110000 }
    },
    "rental": {
      "竿（専用竿, リール）": 1500
    },
    "note": "サイマキ餌5匹付き",
    "visibleShared": true,
    "visibleCharter": true
  },
  "スミイカ": {
    "basePrice": { "men": 11000, "women": 9000, "student": 6500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 88000 },
      "holiday": { "minPeople": 10, "minPrice": 110000 },
      "sunday": { "minPeople": 10, "minPrice": 110000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "シャコ餌付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "テンヤタチウオ": {
    "basePrice": { "men": 10000, "women": 8000, "student": 6500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 80000 },
      "holiday": { "minPeople": 10, "minPrice": 100000 },
      "sunday": { "minPeople": 10, "minPrice": 100000 }
    },
    "rental": {
      "竿（専用竿, リール）": 1500
    },
    "note": "氷付き、イワシ餌10匹650円で別途販売",
    "visibleShared": true,
    "visibleCharter": true
  },
  "マダコ": {
    "basePrice": { "men": 10500, "women": 8500, "student": 6500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 84000 },
      "holiday": { "minPeople": 10, "minPrice": 105000 },
      "sunday": { "minPeople": 10, "minPrice": 105000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "カニ餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "ショートメバル": {
    "basePrice": { "men": 7500, "women": 6500, "student": 4500 },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "活きモエビ・アオイソメ・氷付き",
    "charter": {
      "weekday": { "minPeople": 7, "minPrice": 52500 },
      "holiday": { "minPeople": 10, "minPrice": 75000 },
      "sunday": { "minPeople": 10, "minPrice": 75000 }
    },
    "visibleShared": false,
    "visibleCharter": true
  },
  "鬼カサゴ": {
    "basePrice": { "men": 12500, "women": 11500, "student": 8000 },
    "charter": {
      "weekday": { "minPeople": 10, "minPrice": 125000 },
      "holiday": { "minPeople": 12, "minPrice": 150000 },
      "sunday": { "minPeople": 12, "minPrice": 150000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "サバ餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "深場五目(アコウ or キンメダイ)": {
    "basePrice": { "men": 13000, "women": 11500, "student": 8500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 104000 },
      "holiday": { "minPeople": 10, "minPrice": 130000 },
      "sunday": { "minPeople": 10, "minPrice": 130000 }
    },
    "rental": {
      "竿（通常竿, リール）": 1200,
      "竿（電動リール）": 2500
    },
    "note": "サバ餌・氷付き",
    "visibleShared": true,
    "visibleCharter": true
  },
  "キス＆アナゴリレー": {
    "basePrice": { "men": 11500, "women": 9500, "student": 7500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 92000 },
      "holiday": { "minPeople": 10, "minPrice": 115000 },
      "sunday": { "minPeople": 10, "minPrice": 115000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "夜アナゴ": {
    "basePrice": { "men": 8500, "women": 7000, "student": 5000 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 68000 },
      "holiday": { "minPeople": 10, "minPrice": 85000 },
      "sunday": { "minPeople": 10, "minPrice": 85000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "夜アジ＆カサゴリレー": {
    "basePrice": { "men": 8500, "women": 7000, "student": 5000 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 68000 },
      "holiday": { "minPeople": 10, "minPrice": 85000 },
      "sunday": { "minPeople": 10, "minPrice": 85000 }
    },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "コマセ・餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "夜カサゴ＆メバル": {
    "basePrice": { "men": 7300, "women": 5800, "student": 4500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 58400 },
      "holiday": { "minPeople": 10, "minPrice": 73000 },
      "sunday": { "minPeople": 10, "minPrice": 73000 }
    },
    "rental": { "竿（通常竿, リール）": 600 },
    "note": "餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "一日アミ五目": {
    "basePrice": { "men": 11700, "women": 10000, "student": 7000 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 93600 },
      "holiday": { "minPeople": 10, "minPrice": 117000 },
      "sunday": { "minPeople": 10, "minPrice": 117000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "アミコマセ２ブロック・オキアミ・氷付・追加１ブロック５００円",
    "visibleShared": false,
    "visibleCharter": false
  },
  "タチアミ五目": {
    "basePrice": { "men": 12000, "women": 10000, "student": 7500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 96000 },
      "holiday": { "minPeople": 10, "minPrice": 120000 },
      "sunday": { "minPeople": 10, "minPrice": 120000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "餌・ミンチ・アミコマセ・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "一日ＬＴアジ・メバル": {
    "basePrice": { "men": 10000, "women": 8000, "student": 6500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 80000 },
      "holiday": { "minPeople": 10, "minPrice": 100000 },
      "sunday": { "minPeople": 10, "minPrice": 100000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "コマセ・イカ短・アオイソ・氷付き（ビシアジ＋500円）",
    "visibleShared": false,
    "visibleCharter": true
  },
  "キス（1日船）": {
    "basePrice": { "men": 9500, "women": 7500, "student": 6000 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 76000 },
      "holiday": { "minPeople": 10, "minPrice": 95000 },
      "sunday": { "minPeople": 10, "minPrice": 95000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "アオイソ・餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "カサゴ（1日船）": {
    "basePrice": { "men": 9500, "women": 7500, "student": 6000 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 76000 },
      "holiday": { "minPeople": 10, "minPrice": 95000 },
      "sunday": { "minPeople": 10, "minPrice": 95000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "アオイソ・餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "LTアジ（からリレー）キス・イシモチ・カサゴ・タコ": {
    "basePrice": { "men": 10500, "women": 9000, "student": 6500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 84000 },
      "holiday": { "minPeople": 10, "minPrice": 105000 },
      "sunday": { "minPeople": 10, "minPrice": 105000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "コマセ・イカ短・アオイソ・氷付き（ビシアジ＋500円）\n LTアジ後にキス・イシモチ・カサゴ・タコのどれかを楽しめます。",
    "visibleShared": false,
    "visibleCharter": true
  },
  "LTアジ～マダコ": {
    "basePrice": { "men": 11000, "women": 9000, "student": 6500 },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "コマセ・イカ短・アオイソ・氷付き（ビシアジ＋500円）",
    "visibleShared": false,
    "visibleCharter": false
  },
  "カワハギ～アミ五目": {
    "basePrice": { "men": 12000, "women": 10500, "student": 7500 },
    "charter": {
      "weekday": { "minPeople": 8, "minPrice": 96000 },
      "holiday": { "minPeople": 10, "minPrice": 120000 },
      "sunday": { "minPeople": 10, "minPrice": 120000 }
    },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "アミコマセ・付け餌・氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
  "トラフグ": {
    "basePrice": { "men": 12000, "women": 10000, "student": 8000 },
    "charter": {
      "weekday": { "minPeople": 10, "minPrice": 120000 },
      "holiday": { "minPeople": 12, "minPrice": 144000 },
      "sunday": { "minPeople": 12, "minPrice": 144000 }
    },
    "rental": { "竿（専用竿, リール）": 1500 },
    "note": "餌・氷付き",
    "visibleShared": true,
    "visibleCharter": true
  },
  "マルイカ": {
    "basePrice": { "men": 12000, "women": 10500, "student": 8000 },
    "rental": { "竿（通常竿, リール）": 1200 },
    "note": "氷付き",
    "visibleShared": false,
    "visibleCharter": false
  },
};

const commonRental = {
  "カッパ長靴セット": 600,
  "長靴のみ": 200
};

// 祝日リスト（YYYY-MM-DD）
// テスト用に 2026 年の祝日をここに入れてあります（必要に応じて追加・更新してください）
//
// 【料金タイプの判定ルール】
// - 平日（weekday）: 月〜金曜で祝日でない日
// - 土曜・連休中日（saturday）:
//   * 土曜日
//   * 日曜日で翌日が祝日の場合（連休中日）
//   * 祝日で前後いずれかが休日の場合（連休中日）
// - 日曜・連休最終日（sunday）:
//   * 日曜日で翌日が祝日でない場合（孤立した日曜）
//   * 祝日で前後とも休日でない場合（孤立した祝日）
//   * 祝日で翌日が休日でない場合（連休最終日）
//
// 【単独休日と連続休日の例】
// 単独休日（sunday扱い）:
//   - 2026-01-01（元日、木曜）: 前後が平日 → 孤立した祝日
//   - 2026-01-18（日曜）: 翌日が平日 → 孤立した日曜
//   - 2026-02-11（建国記念の日、水曜）: 前後が平日 → 孤立した祝日
//
// 連続休日（saturday扱い）:
//   - 2026-01-10（土曜）: 通常の土曜
//   - 2026-01-11（日曜）: 翌日が祝日（01-12） → 連休中日
//
// 連続休日最終日（sunday扱い）:
//   - 2026-01-12（成人の日、月曜）: 前日が日曜、翌日が平日 → 連休最終日
//   - 2026-05-03（憲法記念日、日曜）: 翌日が祝日 → 連休初日
//   - 2026-05-04（みどりの日、月曜）: 前後が休日 → 連休中日
//   - 2026-05-05（こどもの日、火曜）: 前日が休日、翌日も休日 → 連休中日
//
// 連続休日最終日（sunday扱い）:
//   - 2026-05-06（振替休日、水曜）: 前日が休日、翌日が平日 → 連休最終日
//   - 2026-09-23（秋分の日、水曜）: 前日が休日、翌日が平日 → 連休最終日
const holidays = [
  '2026-01-01', // 元日（木曜、単独休日）
  '2026-01-12', // 成人の日（月曜、連休最終日：前日1/11が日曜）
  '2026-02-11', // 建国記念の日（水曜、単独休日）
  '2026-02-23', // 天皇誕生日（月曜、単独休日）
  '2026-03-20', // 春分の日（金曜、単独休日）
  '2026-04-29', // 昭和の日（水曜、単独休日）
  '2026-05-04', // みどりの日（月曜、GW連休中日）
  '2026-05-05', // こどもの日（火曜、GW連休中日）
  '2026-05-06', // 振替休日（水曜、GW連休最終日）
  '2026-07-20', // 海の日（月曜、単独休日）
  '2026-08-11', // 山の日（火曜、単独休日）
  '2026-09-21', // 敬老の日（月曜、シルバーウィーク連休初日）
  '2026-09-22', // 国民の休日（火曜、シルバーウィーク連休中日）
  '2026-09-23', // 秋分の日（水曜、シルバーウィーク連休最終日）
  '2026-10-12', // スポーツの日（月曜、単独休日）
  '2026-11-03', // 文化の日（火曜、単独休日）
  '2026-11-23'  // 勤労感謝の日（月曜、単独休日）
];

// Export as ES modules
export { plans, commonRental, holidays };

// Also set on window for backward compatibility (browser)
if (typeof window !== 'undefined') {
  window.plans = plans;
  window.commonRental = commonRental;
  window.holidays = holidays;
}
